...
function ScientificCommittee() {
  ...
  const GetBlogs = (filter) => {
    BlogsService.GetCommittee(filter).then((res) => {
      ListBlogs(res);
    });
  };
  ...
  return (
    <div className="cards-page pdb_50">
      {/* header */}
      <RadiusHeader titleText="scientific_committee" pText="resumes_text" />
      {/* blogs body */}
      <section className="blogsBody mt_50">
        ...
        <div className="container">
          {/* blogs body */}
          <br />
          <div className="blogsItems flex justify_center gap-40 wrap mt_30 mb_120">
            {loadingSkelton
            ...
            {!loadingSkelton && Blogs?.length === 0 && <Empty description={t('no_data')} />}
          </div>
          {/* pagination */}
          {BlogsCount ? (
